...
const ShiftCell = memo(({ day, index, shift, user, loading, onSlotClick, isUnavailable, slotLabel }) => {
    ...
    if (isUnavailable) {
        ...
        statusClass = "bg-white border-gray-200 text-gray-400 hover:border-indigo-300 hover:bg-gray-50/50 cursor-pointer";

        if (isCancelled) {
            statusClass = "bg-gray-50/50 border-gray-100 text-gray-300 cursor-pointer hover:bg-gray-100/80";
        } else if (isUserIn) {
            statusClass = "bg-white border-gray-200 border-l-4 border-l-indigo-500 text-gray-400 hover:border-indigo-300 hover:bg-gray-50/50 cursor-pointer shadow-sm";
        } else if (count >= 4) {
    ...
    return (
        <button
            ...
            className={cn(
                "min-h-[4.5rem] h-auto p-1.5 rounded-lg border transition-all duration-200 flex flex-col items-center justify-center text-xs group focus-visible:ring-2 focus-visible:ring-indigo-500 outline-none",
        ...
        >
            {loading ? (
                ...
                <div className="flex flex-col items-center w-full overflow-hidden">
                    <span className={cn("text-[10px] font-bold mb-1.5 px-2 py-0.5 rounded-full", isUserIn ? "bg-indigo-50 text-indigo-600 border border-indigo-100" : "bg-gray-100 text-gray-500")}>
                        {count}/4
                    </span>
                                ...
                                return (
                                    <span
                                        ...
                                        className={cn(
                                            ...
                                            isResp ? "text-gray-900 font-black" : "text-gray-400 font-medium"
                                        )}
                                        title={`${p.name} ${isResp ? "(Responsable)" : ""}`}
                                    >
                                        {p.name}
                                    </span>
                                );
...
export function ShiftGrid({ weekDates, timeSlots, shifts, user, loading, onSlotClick }) {
    return (
