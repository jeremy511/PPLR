...
const ShiftCell = memo(({ day, index, shift, user, loading, onSlotClick, isUnavailable, slotLabel, zoneColor }) => {
    ...
    let statusStyle = {};
    let statusClass = "bg-muted/30 border-border text-muted-foreground hover:bg-muted/60 cursor-pointer";

    if (isUnavailable) {
        statusClass = "bg-muted border-border text-muted-foreground cursor-not-allowed striped-background pattern-diagonal-lines";
    } else if (shift && (shift.status === "CANCELLED" || count > 0)) {
        const isCancelled = shift.status === "CANCELLED";
        statusClass = "bg-card border-border text-muted-foreground hover:bg-muted/30 cursor-pointer transition-colors";

        if (isCancelled) {
            statusClass = "bg-muted/30 border-border text-muted-foreground cursor-pointer hover:bg-muted/60";
        } else if (isUserIn) {
            // Active user style
            statusClass = "bg-card text-muted-foreground cursor-pointer shadow-sm";
            statusStyle = {
            ...
            statusClass = "bg-card border-border text-muted-foreground cursor-default";
        } else {
    ...
    return (
        <button
            ...
            className={cn(
                ...
                loading ? "bg-muted border-border animate-pulse" : statusClass
            )}
            style={statusStyle}
                                ...
                                return (
                                    <span
                                        ...
                                        className={cn(
                                            ...
                                            isResp ? "font-black" : "text-muted-foreground font-medium"
                                        )}
                                        style={isResp ? {} : {}}
...
export function ShiftGrid({ weekDates, timeSlots, shifts, user, loading, onSlotClick, zoneColor = "#6366f1", isDateUnavailable }) {
    return (
        <div className="p-1 sm:p-4 bg-card rounded-xl shadow-sm border border-border overflow-x-auto custom-scrollbar">
            <div className="min-w-[800px] md:min-w-0">
                {/* Header Row */}
                <div className="grid gap-1 sm:gap-2 mb-4" style={{ gridTemplateColumns: `100px repeat(${weekDates.length}, 1fr)` }}>
                    <div className="font-bold text-muted-foreground text-[10px] uppercase flex items-center justify-center tracking-widest">Hora</div>
                    {weekDates.map((day) => (
                        <div key={day.name} className="flex flex-col items-center pb-2 border-b-2 border-transparent">
                            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-tighter">{day.name.slice(0, 3)}</span>
                            <span className="text-xl font-black text-foreground tabular-nums">{day.date}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="grid gap-1 sm:gap-2">
                    {timeSlots.map((slot, index) => (
                        <div key={index} className="grid gap-1 sm:gap-2" style={{ gridTemplateColumns: `100px repeat(${weekDates.length}, 1fr)` }}>
                            {/* Time Label */}
                            <div className="text-muted-foreground font-bold text-[10px] flex items-center justify-center text-center px-1 tabular-nums leading-tight">
